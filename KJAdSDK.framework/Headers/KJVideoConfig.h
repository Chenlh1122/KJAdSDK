...
typedef NS_ENUM(NSInteger, KJVideoAutoPlayPolicy) {
    KJVideoAutoPlayPolicyWIFI = 0, // WIFI 下自动播放
    KJVideoAutoPlayPolicyAlways = 1, // 总是自动播放，无论网络条件
    KJVideoAutoPlayPolicyNever = 2, // 从不自动播放，无论网络条件
};
@interface KJVideoConfig : NSObject
/**
 视频自动播放策略，默认 KJVideoAutoPlayPolicyAlways,
 选择 KJVideoAutoPlayPolicyNever 策略时，需要开发者调用 KJMediaView 的 play\pause 方法触发视频播、暂停，
 或者开启 userControlEnable 设置，让用户点击 MediaView 控制播放状态
 */
@property (nonatomic, assign) KJVideoAutoPlayPolicy autoPlayPolicy;

/**
 是否静音播放视频广告，视频初始状态是否静音，默认 YES，
 可通过 KJMediaView muteEnable: 方法实时控制播放器j静音状态，
 */
